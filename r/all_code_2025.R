...
# Last modified: April 11, 2025   # run for 2025
...
ssei_aho <- data.frame(year = c(1985:2025), # change most recent year to 2023
                       aho = c(rep(790000, 13), 632000, 720000, rep(696000, 9),
                               ...
                               643360, 643360, 643360, 643360))  #need to update with AHO for new year
...
read_csv("data/fishery/raw_data/SSEI fish ticket data new.csv", guess_max = 50000) %>%  
  clean_names() %>% 
  rename(year = dol_year) %>% 
  filter(species_code == 710, !harvest_code %in% c(17, 43), gear_code != 7) -> fishery_df

# POT LOGBOOK DATA: combine catch and gear logbook data 
read_csv("data/catch_report.csv", guess_max = 50000) %>%  
  clean_names() %>% 
  select(year, project_code, trip_number, effort_number, adfg_number, vessel_name, date_left_port, sell_date, trip_target_species_code_1, 
         trip_target_species_1, trip_target_species_code_2, trip_target_species_2, groundfish_stat_area, 
         groundfish_management_area_code, average_depth_fathoms, species, species_code, disposition, disposition_code, 
         numbers, pounds) %>% 
  filter(species_code == 710, project_code == 617) -> ssei_catch_pot

read_csv("data/gear_report.csv", guess_max = 50000) %>% 
  clean_names() %>% 
  select(year, project_code, trip_number, effort_number, pot_groundline_diameter_inches, pot_spacing_feet, 
         pot_number_of_pots_set, pot_number_of_pots_lost) %>% 
  filter(project_code == 617) %>% 
  ## If number of pots lost is NA then return the number of pots set, otherwise subtract number lost from number set to get those retrieved
  mutate(number_of_pots = case_when(is.na(as.numeric(pot_number_of_pots_lost)) ~ as.numeric(pot_number_of_pots_set),
                               TRUE ~ as.numeric(pot_number_of_pots_set)-as.numeric(pot_number_of_pots_lost))) -> ssei_gear_pot
pot_log_df <- ssei_gear_pot %>% 
  left_join(ssei_catch_pot) %>% 
  select(-c(project_code, pot_number_of_pots_set, pot_number_of_pots_lost, trip_target_species_code_2, trip_target_species_2))
...
fishery_df %>% 
  ...
  # remove the filter if you want to make this non-confidential to see all harvest 
...
# can only keep 2020, 2021, 2022, 2023 due to confidentiality with POTS - check LL (2022-2024 are confidential for LL)
...
pot_log_df %>% 
  ...
  distinct() %>%  
  mutate(Gear = "Pot") %>% 
  filter(between(year, 2018, 2024))  -> pot_trips 
...
fishery_cpue %>% 
  group_by(year) %>% 
  filter(year <= 2021) %>% # 2022-2024 is confidential 
