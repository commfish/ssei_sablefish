...
# for the 2025 SSEI stock assessment run CPUE data, as well as clean up column names, do minor calculations, and overall 
...
ftx <- read.csv("data/fishticket_report.csv") %>% 
  ...
  group_by(across(c(year:ft_species))) %>% 
  summarise(ft_rnd_lbs = sum(ft_rnd_lbs)) %>% ungroup() 
  ## do not use stat_area from fish ticket data -- it's incomplete and wrong! 
...
## run through sql code 

ll_log %>% filter(year == 1999 & trip_number == 364) %>% 
  select(year, project_code, trip_number, effort_number, log_stat_area, log_numbers_sablefish_released, log_numbers_sablefish_released_quota_limit, 
         log_numbers_sablefish_retained, log_rnd_lbs_sablefish_released, log_rnd_lbs_sablefish_released_quota_limit, log_rnd_lbs_sablefish_retained) %>%
  rowwise() %>% 
  mutate(sable_numbers = sum(as.numeric(log_numbers_sablefish_released), as.numeric(log_numbers_sablefish_released_quota_limit), as.numeric(log_numbers_sablefish_retained)), 
         sable_log_pounds = sum(as.numeric(log_rnd_lbs_sablefish_released), as.numeric(log_rnd_lbs_sablefish_released_quota_limit), 
                                          as.numeric(log_rnd_lbs_sablefish_retained), na.rm = TRUE)) %>% 
  full_join(ftx_ll_cpue, by = c("year", "trip_number", "project_code"))  -> sql_test
  #relocate(c(114:120), .before = trip_target_species_combined) 
